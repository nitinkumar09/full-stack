// is file me login signup and logout ka code ha and otp verify before save data of signup by any user
...
//     const otps = otpStore[0];
//     res.status(200).render("otpverify", { otps });
// });

router.post("/otp-success", otpVerificationMiddleware, async (req, res) => {

    const userToken = req.cookies.userToken;
    const userData = jwt.verify(userToken, process.env.SECRET_KEY)
    // console.log("User DATa: ", decode)

    const employeeSchema = await Register.create(userData)
...
router.post("/signup", async (req, res) => { // call back function
    ...
    try {
        ...
        if (password === confirmpassword) // three === isliye lgaye ki type bhi pta chal jaye
        {
            ...
            const userData = jwt.sign(employeeSchema, process.env.SECRET_KEY)

            res.cookie("userToken", userData, {
...
router.get('/success', async (req, res) => {
    try {
        ...
        let user = await Register.findOne({ email: req.user.email });
        ...
        const token = await user.generateAuthToken(); // isse hum middleware bhi kehte ha bich me hi token generate kar rhe h // isse hum jab user login krega to us sme bhi token generate hona chahiye
        ...
        console.log("Error arised", error)
        res.status(400).render("invaliddetail");
    }
